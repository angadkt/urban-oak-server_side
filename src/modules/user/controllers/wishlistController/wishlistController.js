...
export const removeWishlsit = async (req, res) => {
  try {
    ...
    const indexOfElement =  wishlist.products.findIndex((item) => item.productId.toString() === productsId)
    console.log(indexOfElement)
    if(indexOfElement === -1) return res.status(404).json({success:false, message:`error finding index value`})

    wishlist.products.splice(indexOfElement, 1)

    await wishlist.save()
    return res.status(200).json({success:true, message:`product removed`, data:wishlist})
  } catch(err){
